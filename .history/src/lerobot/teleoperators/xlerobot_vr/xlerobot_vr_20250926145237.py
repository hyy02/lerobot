...
class XLerobotVRTeleop(Teleoperator):
    ...
    def __init__(self, config: XLerobotVRTeleopConfig):
        ...
        # 事件处理频率控制（从配置读取）
        self.event_update_interval = 1.0 / self.config.event_update_frequency  # 默认100ms（10Hz）
        self.last_event_update_time = 0.0
        self.control_update_interval = 1.0 / self.config.control_update_frequency  # 默认16.67ms（60Hz）
